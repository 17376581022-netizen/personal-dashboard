...
- **音乐点播**：音乐卡片位于天气右侧。输入歌曲名或歌手后搜索，点击结果即可使用页面内置播放器播放，不会跳转外部平台。搜索使用无需 API Key 的 iTunes Search API，并在普通跨域请求失败时自动切换 JSONP 兼容通道，以适配手机网络和不同来源；播放内容是曲库提供的合法试听片段。受版权、地区或运营商网络限制，部分歌曲仍可能没有试听资源。
