...
Personal Dashboard 是一个无需安装的个人生活仪表盘。除自动天气和账号同步需要联网外，其他模块都可离线使用。它把天气、当天待办、习惯打卡、重要日期、快捷链接、音乐、项目进度、每日记录和月度回顾放在一个页面中。
...
账号区仅保留邮箱加密码的注册和登录。邮箱确认已关闭，注册后可直接登录；手机号短信和微信登录入口已移除。
...
- **天气**：输入中文城市名（例如“杭州”“上海”“北京”），点击“保存城市”或按 Enter；也可以直接点击北京、上海、杭州、南京、广州等热门城市。城市搜索和天气请求带有超时、最多三次自动重试及本地缓存回退。点击“刷新天气”可立即更新；页面打开时会后台更新，并每 30 分钟自动刷新一次。
...
- **音乐点播**：音乐卡片位于天气右侧，内置 500 首原创程序生成式 R&B 曲目。可按曲名、编号、风格、BPM 或调性筛选，也可以随机播放。点击曲目后，浏览器使用 Web Audio 在本机实时合成声音并自动循环；播放不请求网易云、QQ 音乐、iTunes 等第三方曲库，不下载音频，也不会跳转外部平台。由于声音由代码即时生成，它不是第三方商业歌曲或其改编版本。
