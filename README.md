...
- **音乐点播**：音乐卡片位于天气右侧。输入歌曲名或歌手后搜索，点击结果即可使用页面内置播放器播放，不会跳转外部平台。桌面端普通请求失败时会自动切换 JSONP 兼容通道；移动端和 `file://` 页面会优先使用兼容通道，并缓存最近一次成功搜索结果。播放内容是曲库提供的合法试听片段；受版权、地区或运营商网络限制，部分歌曲仍可能没有试听资源。
