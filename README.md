...
Personal Dashboard 是一个无需安装的个人生活仪表盘。除自动天气需要联网外，其他模块都可离线使用。它把天气、当天待办、习惯打卡、重要日期、快捷链接、项目进度、每日记录和月度回顾放在一个页面中。
...
Supabase 托管项目默认可能要求注册后验证邮箱。部署完成后，请在 **Authentication → URL Configuration** 中把 GitHub Pages 地址设置为 Site URL，并加入 Redirect URLs。
...
- **天气**：输入英文或拼音城市名（例如 `Hangzhou`、`Shanghai`、`Beijing`），点击“保存城市”或按 Enter。系统先通过 Open-Meteo Geocoding API 搜索城市，再通过 Forecast API 获取当前天气。点击“刷新天气”可立即更新；页面打开时会后台更新，并每 30 分钟自动刷新一次。
...
所有数据都保存在当前浏览器的 `localStorage` 中，不会上传到服务器。刷新页面或关闭浏览器后数据仍会保留，但清理浏览器网站数据、切换浏览器或使用隐私模式可能导致数据不可见或被清除。
